...
import IntroduceTheProgramDialog from "./IntroduceTheProgramDialog";
import { toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
import {searchByPageMessage, deleteItemMessage, getItemByIdMessage} from "./IntroduceTheProgramService";
...
class IntroduceTheProgramTable extends Component {
  state = {
    rowsPerPage: 10,
    page: 0,
    data: [],
  ...
  search() {
    ...
    searchByPageMessage(searchObject, this.state.page, this.state.rowsPerPage).then(
      ({ data }) => {
        this.setState({
  ...
  handleConfirmationResponse = () => {
    let { t } = this.props;
    deleteItemMessage(this.state.id).then((res) => {
      if (res.data == true) {
        ...
        render: (rowData) => (
          <MaterialButton
            ...
            onSelect={(rowData, method) => {
              if (method === 0) {
                getItemByIdMessage(rowData.id).then(({data}) => {
                    this.setState({
                        item: data,
                        shouldOpenEditorDialog: true
                    })
...
export default IntroduceTheProgramTable;
